...
void *postorder(struct node *root) {
    ...
    if (root != NULL) {
        ...
        // Step 1: First, recursively traverse the LEFT subtree
        postorder(root->left);

        // Step 2: Then, recursively traverse the RIGHT subtree
        postorder(root->right);

        // Step 3: Finally, process the ROOT node (print its data)
        printf("%d ", root->data);
    }
}
int main(){
